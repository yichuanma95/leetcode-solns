...
Solution runtime: 44ms, faster than 97.21% of Python3 submissions
'''
...
class Solution:
    def postorder(self, root: 'Node') -> List[int]:
        vals = []
        self.gather_values(root, vals)
        return vals
    
    def gather_values(self, node, vals):
        if not node:
            return
        for child in node.children:
            self.gather_values(child, vals)
