...
class Solution:
    # This solution actually "simulates" the robot on the grid based on its given moves.
    def judgeCircle(self, moves: str) -> bool:
        pos = [0, 0]
        for move in moves:
            self.handle_move(pos, move)
        return pos[0] == 0 and pos[1] == 0
            
    def handle_move(self, pos, move):
        if move == 'R':
            pos[0] += 1
        elif move == 'L':
            pos[0] -= 1
    ...
    # This shorter solution counts each distinct move in moves. The robot is back at the origin
    # if there are as many ups as downs and as many lefts as rights.
    def judgeCircle_short(self, moves: str) -> bool:
        move_count = Counter(moves)
        return move_count['L'] == move_count['R'] and move_count['U'] == move_count['D']
