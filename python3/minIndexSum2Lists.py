'''
Problem 599: Minimum Index Sum of Two Lists

Suppose Andy and Doris want to choose a restaurant for dinner, and they both have a list
of favorite restaurants represented by strings.

You need to help them find out their common interest with the least list index sum. If there is
a choice tie between answers, output all of them with no order requirement. You could assume
there always exists an answer.

Example 1:
Input:
["Shogun", "Tapioca Express", "Burger King", "KFC"]
["Piatti", "The Grill at Torrey Pines", "Hungry Hunter Steakhouse", "Shogun"]
Output: ["Shogun"]
Explanation: The only restaurant they both like is "Shogun".

Example 2:
Input:
["Shogun", "Tapioca Express", "Burger King", "KFC"]
["KFC", "Shogun", "Burger King"]
Output: ["Shogun"]
Explanation: The restaurant they both like and have the least index sum is "Shogun" with index
sum 1 (0+1).

Note:s
1. The length of both lists will be in the range of [1, 1000].
2. The length of strings in both lists will be in the range of [1, 30].
3. The index is starting from 0 to the list length minus 1.
4. No duplicates in both lists.

Solution runtime: 152ms, faster than 97.26% of Python3 submissions

Solution memory usage: 12.9 MB, less than 100% of Python3 submissions
'''

class Solution:
    def findRestaurant(self, list1: List[str], list2: List[str]) -> List[str]:
        restoDict = {}
        leastIndexSum: int = len(list1) + len(list2) + 1
        result: List[str] = []
        
        for i in range(len(list1)):
            restoDict[list1[i]] = i
        for i in range(len(list2)):
            if list2[i] in restoDict:
                if i + restoDict[list2[i]] < leastIndexSum:
                    leastIndexSum = i + restoDict[list2[i]]
                    del result
                    result = [list2[i]]
                elif i + restoDict[list2[i]] == leastIndexSum:
                    result.append(list2[i])
    
        return result
