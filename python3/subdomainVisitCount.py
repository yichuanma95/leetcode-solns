...
level, "discuss.leetcode.com". When we visit a domain like "discuss.leetcode.com", we will
also visit the parent domains "leetcode.com" and "com" implicitly.
...
Solution runtime: 56ms, faster than 96.75% of Python3 submissions
'''
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for domain in cpdomains:
            row = domain.split()
            times: int = int(row[0])
            domainList: List[str] = row[1].split(".")
            for i in range(len(domainList)):
                url: str = '.'.join(domainList[i:])
                if url in domains:
                    domains[url] += times
                else:
                    domains[url] = times
        ...
        for domain in domains.keys():
            result.append(str(domains[domain]) + " " + domain)
